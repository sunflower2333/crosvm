...
use vm_control::shared_allocation::{DynamicMappingWindow, ExternalMappingLedger,
    ExternalMappingReceipt, ranges_overlap};
...
pub struct VirtioPciDevice {
    ...
    // Immutable BAR backing that must be installed after BAR allocation but before VM start.
    pre_start_shared_memory: Option<PreStartSharedMemory>,
...
impl PciDevice for VirtioPciDevice {
    ...
    fn allocate_device_bars(
        ...
        allocate_device_bars(
            self,
            |size: u64, alloc: Alloc, alloc_option: &AllocOptions| {
                if std::env::var_os("CROSVM_DRM2KGSL_BAR_PREBACKED").is_some() {
                    info!(
                        "GPU-BAR-ALLOC: device={} request size={:#x} alloc={:?} options={:?}",
                        device_type, size, alloc, alloc_option,
                    );
                }
                ...
                if std::env::var_os("CROSVM_DRM2KGSL_BAR_PREBACKED").is_some() {
                    info!(
                        "GPU-BAR-ALLOC: device={} result address={:#x} size={:#x}",
                        device_type, allocated, size,
                    );
                }
                Ok(allocated)
            },
    ...
    fn prepare_device_memory(
        &mut self,
        vm: &mut dyn Vm,
        resources: &mut SystemAllocator,
    ) -> std::result::Result<Option<vm_control::VmMappedMemoryRegion>, PciDeviceError> {
        let Some(pre_start) = self.pre_start_shared_memory.take() else {
            return Ok(None);
        };
        let (region, _, _) = pre_start
            .source
            .map_without_gralloc(Protection::read_write())
            .context("failed to map pre-start shared memory source")
            .map_err(PciDeviceError::MmioSetup)?;
        let mapped_region = vm_control::sys::prepare_shared_memory_region_at_offset(
            vm,
            resources,
            pre_start.alloc,
            pre_start.cache,
            pre_start.guest_offset,
            Some(region),
        )
        .context("failed to install pre-start shared memory region")
        .map_err(PciDeviceError::MmioSetup)?;
        Ok(Some(mapped_region))
    }
    ...
    fn write_config_register(&mut self, reg_idx: usize, offset: u64, data: &[u8]) {
        // The drm2kgsl pre-backed BAR is already mapped in Gunyah stage-2. The PCI
        // configuration object locks only the address bits, so firmware can still write all
        // ones and read the normal size mask during BAR probing. Do not short-circuit this
        // call: doing so would make the probe read back the live address and can prevent UEFI
        // from registering a bootable device.
        let prebacked_bar = std::env::var_os("CROSVM_DRM2KGSL_BAR_PREBACKED").is_some()
            && (reg_idx == 4 + SHMEM_BAR_NUM || reg_idx == 4 + SHMEM_BAR_NUM + 1);
        ...
        if prebacked_bar {
            info!(
                "GPU-BAR-CONFIG: applied masked prebacked BAR2 write reg={} offset={:#x} len={}",
                reg_idx,
                offset,
                data.len(),
            );
    ...
    F: FnMut(u64, Alloc, &AllocOptions) -> std::result::Result<u64, PciDeviceError>,
{
    ...
    let configs = if !configs.is_empty() {
        ...
        vm_requester = Some(Box::new(VmRequester::new(
            vm_memory_client,
            alloc,
            prepare_type,
            region.length,
    ...
    for config in configs {
        ...
        if std::env::var_os("CROSVM_DRM2KGSL_BAR_PREBACKED").is_some() {
            info!(
                "GPU-BAR-CONFIG: device={} bar={} address={:#x} size={:#x} 64bit={} prefetchable={}",
                virtio_pci_device.device.device_type(),
                config.bar_index(),
                config.address(),
                config.size(),
                config.is_64bit_memory(),
                config.is_prefetchable(),
            );
        }
        ...
        if std::env::var_os("CROSVM_DRM2KGSL_BAR_PREBACKED").is_some()
            && config.bar_index() == SHMEM_BAR_NUM
        {
            virtio_pci_device
                .config_regs
                .lock_bar_address(config.bar_index())
                .map_err(|e| PciDeviceError::IoRegistrationFailed(device_addr, e))?;
            info!(
                "GPU-BAR-CONFIG: locked prebacked shared-memory BAR={} at {:#x}",
                config.bar_index(),
                device_addr,
            );
        }
    ...
    if virtio_pci_device
        ...
        .is_some()
    {
        if let Some(mut requester) = vm_requester {
            virtio_pci_device.pre_start_shared_memory = requester.take_eager();
            virtio_pci_device.device.set_shared_memory_mapper(requester);
        }
        ...
        if std::env::var_os("CROSVM_DRM2KGSL_BAR_PREBACKED").is_some() {
            for bar in virtio_pci_device.config_regs.get_bars() {
                info!(
                    "GPU-BAR-CONFIG: final bar={} address={:#x} size={:#x} 64bit={} prefetchable={}",
                    bar.bar_index(),
                    bar.address(),
                    bar.size(),
                    bar.is_64bit_memory(),
...
struct VmRequester {
    ...
    alloc: Alloc,
    mappings: BTreeMap<u64, VmRequestedMapping>,
    prepare_type: SharedMemoryPrepareType,
    prepared: bool,
    prepared_cache: Option<MemCacheType>,
    external_window: DynamicMappingWindow,
    external_ledger: ExternalMappingLedger,
    // Keep even an uncertain/failed SHARE's source alive until teardown.
    external_backings: BTreeMap<u64, base::SafeDescriptor>,
}
...
impl VmRequester {
    fn new(
        ...
        let reserved_prefix = match &prepare_type {
            SharedMemoryPrepareType::SingleMappingEager(_, source, offset) =>
                offset.checked_add(source.size()).unwrap_or(u64::MAX),
            SharedMemoryPrepareType::SingleMappingOnFirst(_) => bar_size,
            SharedMemoryPrepareType::DynamicPerMapping => 0,
        };
        Self {
            vm_memory_client,
            alloc,
            ...
            prepared_cache: None,
            external_window: DynamicMappingWindow {
                bar_size, reserved_prefix, alignment: base::pagesize() as u64,
            },
            external_ledger: ExternalMappingLedger::default(),
            external_backings: BTreeMap::new(),
        }
    }

    fn take_eager(&mut self) -> Option<PreStartSharedMemory> {
        let prepare_type = std::mem::replace(
            &mut self.prepare_type,
            SharedMemoryPrepareType::DynamicPerMapping,
        );
        match prepare_type {
            SharedMemoryPrepareType::SingleMappingEager(cache, source, guest_offset) => {
                self.prepared = true;
                self.prepared_cache = Some(cache);
                Some(PreStartSharedMemory {
                    alloc: self.alloc,
                    cache,
                    source,
                    guest_offset,
            ...
            other => {
                self.prepare_type = other;
                None
            }
        }
    }

    fn prepare_lazy(&mut self) -> anyhow::Result<()> {
        if self.prepared {
            return Ok(());
        }
        if let SharedMemoryPrepareType::SingleMappingOnFirst(cache) = &self.prepare_type {
            let cache = *cache;
            debug!("lazy prepare_shared_memory_region with {:?}", cache);
            self.vm_memory_client
                .prepare_shared_memory_region(self.alloc, cache, None)
    ...
    fn validate_cache(&self, cache: MemCacheType) -> anyhow::Result<()> {
        if cache == MemCacheType::CacheNonCoherent
            && self.prepared_cache == Some(MemCacheType::CacheCoherent)
        {
            error!(
                "invalid request to map with CacheNonCoherent for device with prepared CacheCoherent memory"
            );
    ...
    fn validate_unoccupied(&self, offset: u64, size: u64) -> anyhow::Result<()> {
        if size == 0 || !offset.checked_add(size)
                .is_some_and(|end| end <= self.external_window.bar_size)
            || self.external_ledger.overlaps(offset, size)
            || self.mappings.iter().any(|(start, map)| ranges_overlap(offset, size, *start, map.size)) {
            return Err(anyhow!("invalid or occupied shared-memory range"));
        }
        Ok(())
    }
}
...
impl SharedMemoryMapper for VmRequester {
    fn external_mapping_window(&self) -> Option<DynamicMappingWindow> {
        Some(self.external_window)
    }

    fn add_external_mapping(&mut self, source: VmMemorySource, offset: u64,
        prot: Protection, cache: MemCacheType) -> anyhow::Result<ExternalMappingReceipt> {
        let size = source.size();
        self.validate_unoccupied(offset, size)?;
        if !self.external_window.permits(offset, size) {
            return Err(anyhow!("external allocation requires an aligned independent BAR suffix"));
        }
        let VmMemorySource::Descriptor { descriptor, offset: fd_offset, .. } = &source else {
            return Err(anyhow!("external allocation requires an owned descriptor"));
        };
        if *fd_offset % self.external_window.alignment != 0 {
            return Err(anyhow!("unaligned external descriptor offset"));
        }
        let retained = base::clone_descriptor(descriptor)
            .context("retain external allocation backing")?;
        self.prepare_lazy()?;
        // This creates a separate memory slot outside the prepared prefix.
        // Its cache policy comes from the imported DMA allocation, never from
        // the unrelated eager arena's established stage-2 mapping.
        let receipt = self.external_ledger.reserve(self.external_window, offset, size)
            .context("external mapping reservation failed")?;
        self.external_backings.insert(offset, retained);
        let mapped = self.vm_memory_client.register_memory_for_blob(
            source,
            VmMemoryDestination::ExistingAllocationDynamic { allocation: self.alloc, offset },
            prot, cache, VmAccept::Sync,
        );
        let (id, handle) = match mapped {
            Ok(value) => value,
            Err(error) => {
                self.external_ledger.quarantine(receipt);
                return Err(error).context(vm_control::shared_allocation::ExternalMappingFailure { receipt });
            }
        };
        self.mappings.insert(offset, VmRequestedMapping { id, size });
        if handle.is_some() || !self.external_ledger.commit(receipt) {
            self.external_ledger.quarantine(receipt);
            return Err(anyhow!("external mapping lacks completed guest acceptance"))
                .context(vm_control::shared_allocation::ExternalMappingFailure { receipt });
        }
        Ok(receipt)
    }

    fn external_mapping_live(&self, receipt: ExternalMappingReceipt) -> bool {
        self.external_ledger.is_live(receipt)
            && self.mappings.get(&receipt.offset).is_some_and(|map| map.size == receipt.size)
    }

    fn remove_external_mapping(&mut self, receipt: ExternalMappingReceipt) -> anyhow::Result<()> {
        if !self.external_ledger.begin_remove(receipt) {
            return Err(anyhow!("stale external mapping identity"));
        }
        let id = self.mappings.get(&receipt.offset).context("missing external mapping")?.id;
        self.vm_memory_client.unregister_memory(id)
            .context("external guest release/unshare failed; mapping retained")?;
        if !self.external_ledger.complete_remove(receipt) {
            return Err(anyhow!("external mapping retirement identity mismatch"));
        }
        self.mappings.remove(&receipt.offset);
        self.external_backings.remove(&receipt.offset);
        Ok(())
    }
    ...
    fn remove_mapping(&mut self, offset: u64) -> anyhow::Result<()> {
        if self.external_ledger.overlaps(offset, 1) {
            return Err(anyhow!("external unmap requires exact mapping receipt"));
        }
        let id = self.mappings.get(&offset).context("invalid offset")?.id;
        self.vm_memory_client
            .unregister_memory(id)
            .context("unregister_memory failed")?;
        // Keep the bookkeeping entry until the hypervisor confirms the
        // unregister.  A failed unregister is retryable; removing it first
        // would turn the next UNMAP into an "invalid offset" and leak the
        // mapping permanently.
