...
use vm_control::shared_allocation::{DynamicMappingWindow, ExternalMappingReceipt};
...
pub enum SharedMemoryPrepareType {
    ...
    /// Before the VM starts, back a fixed subrange of the SharedMemoryRegion with this source.
    /// Later device commands only publish offsets into that immutable backing; they must not
    /// replace its pages after the hypervisor has installed its stage-2 mapping. The offset is
    /// relative to the BAR.
    SingleMappingEager(MemCacheType, VmMemorySource, u64),
...
pub trait SharedMemoryMapper: Send {
    /// Exact independent BAR suffix. None means this mapper cannot prove one.
    fn external_mapping_window(&self) -> Option<DynamicMappingWindow> { None }

    /// Import a real descriptor via runtime SHARE/guest ACCEPT, not fixed mmap.
    /// Receipt is published only after mapping succeeds; it is not a lease.
    fn add_external_mapping(&mut self, _source: VmMemorySource, _offset: u64,
        _prot: Protection, _cache: MemCacheType) -> Result<ExternalMappingReceipt> {
        Err(anyhow!("independent external mappings unsupported"))
    }

    fn external_mapping_live(&self, _receipt: ExternalMappingReceipt) -> bool { false }

    fn remove_external_mapping(&mut self, _receipt: ExternalMappingReceipt) -> Result<()> {
        Err(anyhow!("independent external mappings unsupported"))
    }
