...
use super::display_color_protocol::{GetDisplayColor, SetResourceColor, DisplayColorResponse,
    CMD_GET_DISPLAY_COLOR, CMD_SET_RESOURCE_COLOR, RESP_DISPLAY_COLOR,
    SetTargetTransform, CMD_SET_TARGET_TRANSFORM};
use super::shared_allocation_protocol::{SharedAllocationHeader, SharedAllocationDiscovery,
    CMD_DISCOVER_SHARED_ALLOCATION, RESP_SHARED_ALLOCATION,
    SharedAllocationRequest, SharedAllocationResponse, SharedAllocationAck,
    CMD_ALLOCATE_SHARED_ALLOCATION, CMD_ACK_SHARED_ALLOCATION, CMD_DESTROY_SHARED_ALLOCATION,
    RESP_ALLOCATED_SHARED_ALLOCATION, SharedOwnerRequest, SharedOwnerResponse,
    RecoverableAllocationRequest, CMD_QUERY_SHARED_OWNER, CMD_ALLOCATE_RECOVERABLE,
    CMD_CLEANUP_SHARED_OWNER, RESP_SHARED_OWNER};
...
pub use super::control_header::virtio_gpu_ctrl_hdr;
...
#[derive(Copy, Clone)]
pub enum GpuCommand {
    QuerySharedOwner(SharedOwnerRequest),
    AllocateRecoverable(RecoverableAllocationRequest),
    CleanupSharedOwner(SharedOwnerRequest),
    DiscoverSharedAllocation(SharedAllocationHeader),
    AllocateSharedAllocation(SharedAllocationRequest),
    AcknowledgeSharedAllocation(SharedAllocationAck),
    DestroySharedAllocation(SharedAllocationHeader),
    GetDisplayColor(GetDisplayColor),
    SetResourceColor(SetResourceColor),
    SetTargetTransform(SetTargetTransform),
...
impl fmt::Debug for GpuCommand {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        use self::GpuCommand::*;
        match self {
            QuerySharedOwner(_) => f.debug_struct("QuerySharedOwner").finish(),
            AllocateRecoverable(_) => f.debug_struct("AllocateRecoverable").finish(),
            CleanupSharedOwner(_) => f.debug_struct("CleanupSharedOwner").finish(),
            DiscoverSharedAllocation(_) => f.debug_struct("DiscoverSharedAllocation").finish(),
            AllocateSharedAllocation(_) => f.debug_struct("AllocateSharedAllocation").finish(),
            AcknowledgeSharedAllocation(_) => f.debug_struct("AcknowledgeSharedAllocation").finish(),
            DestroySharedAllocation(_) => f.debug_struct("DestroySharedAllocation").finish(),
            GetDisplayColor(_) => f.debug_struct("GetDisplayColor").finish(),
            SetResourceColor(_) => f.debug_struct("SetResourceColor").finish(),
            SetTargetTransform(_) => f.debug_struct("SetTargetTransform").finish(),
...
impl GpuCommand {
    ...
    pub fn decode(cmd: &mut Reader) -> Result<GpuCommand, GpuCommandDecodeError> {
        ...
        Ok(match hdr.type_.into() {
            CMD_QUERY_SHARED_OWNER => QuerySharedOwner(cmd.read_obj()?),
            CMD_ALLOCATE_RECOVERABLE => AllocateRecoverable(cmd.read_obj()?),
            CMD_CLEANUP_SHARED_OWNER => CleanupSharedOwner(cmd.read_obj()?),
            CMD_DISCOVER_SHARED_ALLOCATION => DiscoverSharedAllocation(cmd.read_obj()?),
            CMD_ALLOCATE_SHARED_ALLOCATION => AllocateSharedAllocation(cmd.read_obj()?),
            CMD_ACK_SHARED_ALLOCATION => AcknowledgeSharedAllocation(cmd.read_obj()?),
            CMD_DESTROY_SHARED_ALLOCATION => DestroySharedAllocation(cmd.read_obj()?),
            CMD_GET_DISPLAY_COLOR => GetDisplayColor(cmd.read_obj()?),
            CMD_SET_RESOURCE_COLOR => SetResourceColor(cmd.read_obj()?),
            CMD_SET_TARGET_TRANSFORM => SetTargetTransform(cmd.read_obj()?),
    ...
    pub fn ctrl_hdr(&self) -> &virtio_gpu_ctrl_hdr {
        use self::GpuCommand::*;
        match self {
            QuerySharedOwner(info) | CleanupSharedOwner(info) => &info.query.hdr,
            AllocateRecoverable(info) => &info.allocation.query.hdr,
            DiscoverSharedAllocation(info) => &info.hdr,
            AllocateSharedAllocation(info) => &info.query.hdr,
            AcknowledgeSharedAllocation(info) => &info.query.hdr,
            DestroySharedAllocation(info) => &info.hdr,
            GetDisplayColor(info) => &info.hdr,
            SetResourceColor(info) => &info.query.hdr,
            SetTargetTransform(info) => &info.query.hdr,
...
pub enum GpuResponse {
    OkSharedOwner(SharedOwnerResponse),
    OkSharedAllocation(SharedAllocationDiscovery),
    OkAllocatedSharedAllocation(SharedAllocationResponse),
    OkDisplayColor(DisplayColorResponse),
    OkNoData,
...
impl Display for GpuResponse {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match self {
            OkDisplayColor(_) => write!(f, "ok display color"),
            OkSharedOwner(_) => write!(f, "ok shared owner recovery"),
            OkSharedAllocation(_) => write!(f, "ok shared allocation discovery"),
            OkAllocatedSharedAllocation(_) => write!(f, "ok native allocation"),
...
impl GpuResponse {
    ...
    pub fn encode(
        ...
        let len = match *self {
            GpuResponse::OkSharedOwner(info) => info.encode(hdr, resp)?,
            GpuResponse::OkSharedAllocation(info) => info.encode(hdr, resp)?,
            GpuResponse::OkAllocatedSharedAllocation(mut info) => {
                info.query.hdr = hdr;
                resp.write_obj(info)?;
                size_of::<SharedAllocationResponse>()
            }
            GpuResponse::OkDisplayColor(mut info) => {
                info.query.hdr = hdr;
                resp.write_obj(info)?;
                size_of::<DisplayColorResponse>()
            }
    ...
    pub fn get_type(&self) -> u32 {
        match self {
            GpuResponse::OkDisplayColor(_) => RESP_DISPLAY_COLOR,
            GpuResponse::OkSharedOwner(_) => RESP_SHARED_OWNER,
            GpuResponse::OkSharedAllocation(_) => RESP_SHARED_ALLOCATION,
            GpuResponse::OkAllocatedSharedAllocation(_) => RESP_ALLOCATED_SHARED_ALLOCATION,
